...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.unsoftDeleteUserStmt, err = db.PrepareContext(ctx, unsoftDeleteUser); err != nil {
		return nil, fmt.Errorf("error preparing query UnsoftDeleteUser: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.unsoftDeleteUserStmt != nil {
		if cerr := q.unsoftDeleteUserStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing unsoftDeleteUserStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		unsoftDeleteUserStmt: q.unsoftDeleteUserStmt,
